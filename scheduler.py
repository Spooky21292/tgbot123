...
async def check_reminders(bot: Bot, db: Database, timezone: str) -> None:
...
def setup_scheduler(bot: Bot, db: Database, timezone: str) -> AsyncIOScheduler:
    scheduler = AsyncIOScheduler(timezone=timezone)
    scheduler.add_job(check_reminders, "interval", minutes=2, args=[bot, db, timezone])
    scheduler.start()
