...
from keyboards import main_keyboard, mini_app_keyboard, task_actions_keyboard
...
async def send_welcome(message: Message, mini_app_url: str) -> None:
    text = (
        "Привет! Я StudyPing 📌\n"
    ...
    if mini_app_url:
        await message.answer("Открой веб-версию бота 👇", reply_markup=mini_app_keyboard(mini_app_url))
...
def remember_user(message: Message, db: Database) -> None:
    if not message.from_user:
        return
    db.upsert_user(message.from_user.id, message.from_user.username)


def register_handlers(dp: Dispatcher, db: Database, timezone: str, admin_id: int, mini_app_url: str) -> None:
    @dp.message(Command("start"))
    async def start_handler(message: Message) -> None:
        remember_user(message, db)
        await send_welcome(message, mini_app_url)
    ...
    async def app_handler(message: Message) -> None:
        remember_user(message, db)
        if not mini_app_url:
            await message.answer("❌ MINI_APP_URL не задан. Добавь ссылку на сайт в .env")
            return
    ...
    async def stats_handler(message: Message) -> None:
        ...
        text = "\n".join([f"{uid} @{username}" if username else f"{uid} @-" for uid, username in rows])
...
async def main() -> None:
    ...
    register_handlers(dp, db, cfg.timezone, cfg.admin_id, cfg.mini_app_url)
