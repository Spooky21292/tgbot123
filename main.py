...
async def send_welcome(message: Message, web_app_url: str) -> None:
    text = (
        "Привет! Я StudyPing 📌\n"
...
def register_handlers(
    ...
    async def start_handler(message: Message) -> None:
        ...
        await send_welcome(message, web_app_url)
    ...
    async def mini_handler(message: Message) -> None:
        remember_user(message, db)
        if not web_app_url:
            await message.answer(
                "⚠️ Mini App URL пока не задан. Укажи WEB_APP_URL в .env и перезапусти бота."
            )
            return

        await message.answer(
            "🚀 Открывай Mini App кнопкой ниже:",
            reply_markup=mini_app_keyboard(web_app_url),
        )
...
async def main() -> None:
    ...
    register_handlers(dp, db, cfg.timezone, cfg.admin_id, cfg.web_app_url)
