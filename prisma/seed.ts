...
const lessonCaseByCode: Record<string, string> = {
  '1.1': 'Вы впервые честно фиксируете, как именно принимаете денежные решения за неделю.',
  '1.2': 'В течение дня появляется 3–4 мелких траты, и важно не потерять контроль из-за “это же немного”.',
  '1.3': 'После сложного дня хочется быстро купить что-то “для компенсации”, хотя это ломает план.',
  '1.4': 'Нужно составить спокойный план на неделю, чтобы он не развалился уже на второй день.',
  '2.1': 'Есть несколько целей одновременно, и надо выбрать приоритет без ощущения потери.',
  '2.2': 'Повторяющиеся платежи незаметно съедают бюджет, хотя каждая сумма отдельно выглядит небольшой.',
  '2.3': 'Вы тестируете 3 простых правила и смотрите, какие реально работают в вашем графике.',
  '2.4': 'Разбираете реальную ситуацию из жизни и выбираете не идеальное, а рабочее решение.',
  '3.1': 'Крупная цель кажется далёкой, пока вы не переводите её в сумму, срок и ежемесячный шаг.',
  '3.2': 'Неожиданный расход проверяет, есть ли у вас запас прочности и заранее продуманный сценарий.',
  '3.3': 'После отката важно не бросить систему, а спокойно вернуться в ритм без самокритики.',
  '3.4': 'Нужно настроить короткий обзор недели, чтобы видеть прогресс и вовремя корректировать план.',
  '4.1': 'Разговор о деньгах с близкими может быть спокойным, если есть понятная структура диалога.',
  '4.2': 'Вы собираете личную систему на 90 дней и выбираете минимум действий, которые реально выдержать.',
  '4.3': 'Появляется риск снижения дохода, и вы заранее определяете порядок сокращения расходов.',
  '4.4': 'Фиксируете итоги курса и переводите знания в конкретный следующий шаг на ближайший месяц.'
};
...
function makeLesson(
  ...
  const lessonCodeMatch = title.match(/Урок\s+(\d\.\d)/);
  const lessonCode = lessonCodeMatch?.[1] ?? '0.0';
  const lessonCase = lessonCaseByCode[lessonCode] ?? 'Ситуация разбирается на примере типичного финансового решения из повседневной жизни.';
...
Представьте ситуацию: ${lessonCase} Обычно это приводит к циклу: напряжение → импульсивное решение → краткое облегчение → новая тревога. Когда вы применяете рамку именно этого урока (${lessonCode}), решение становится структурным и повторяемым.

## Разбор урока по шагам

1. **Что происходит сейчас** — называем проблему простыми словами в контексте урока ${lessonCode} и фиксируем стартовую ситуацию без самообвинения.
2. **Почему так происходит** — выделяем триггеры именно для темы «${moduleSummary}»: спешка, усталость, давление окружения, отсутствие понятной структуры.
3. **Как действовать спокойнее** — вводим 1–2 правила под текущий модуль `${moduleTitle}`, которые реально соблюдать в обычном ритме жизни.
4. **Как проверить результат** — через 7 дней сверяем, стало ли меньше хаоса и легче ли принимать решения по теме урока ${lessonCode}.
