...
function makeLesson(
  ...
  const lessonTopic = title.includes(':') ? title.split(':').slice(1).join(':').trim() : title;
  const content = `# ${title}
...
## Ключевая идея именно этого урока

Тема урока: **${lessonTopic}**.  
Практический фокус: **${description}**.

В этом занятии мы не повторяем весь курс, а разбираем именно этот узкий шаг: какие решения чаще всего ломаются в точке «${lessonTopic}», какие сигналы предупреждают об ошибке заранее и как собрать одно рабочее правило, которое можно проверить уже сегодня.
...
2. **Почему так происходит** — выделяем триггеры именно для шага «${lessonTopic}»: спешка, усталость, давление окружения, отсутствие понятной структуры.
3. **Как действовать спокойнее** — вводим 1–2 правила под задачу урока «${lessonTopic}», которые реально соблюдать в обычном ритме жизни.
...
## Практика по теме урока «${lessonTopic}»

1. Выпишите одну реальную ситуацию за последние 7 дней, где вам пригодилась бы логика этого урока.
2. Переформулируйте её в формат «сигнал → действие → ожидаемый результат».
3. Проведите маленький эксперимент в течение недели и зафиксируйте, как изменилось качество решений.
